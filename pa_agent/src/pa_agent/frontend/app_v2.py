...
def calculate_streak(df, mood):
    ...
    for value in is_mood:
        if value: # om det matchar med mood som skickats in
            current_streak += 1
...
def show_kpis(df, mood):
    # Antalet procent positivt/negativt
    total = len(df)
...
def show_trend(df):
    ...
    last_3_posts = df.tail(3)['mood'].value_counts()
    pos_count = last_3_posts.get('positivt', 0) # Krashar annars om pos / neg inte finns
    neg_count = last_3_posts.get('negativt', 0)
